...
struct SpendingGraphView: View {
    ...
    private func calculateMonthlyDataWithOffset(calendar: Calendar, today: Date, offset: Int) -> [(String, Double)] {
        ...
        // Calculate the exact 6-month date range we're displaying
        var displayMonths: [Date] = []
        for i in 0..<6 {
            let monthDate = cal.date(byAdding: .month, value: -i, to: adjustedTodayStart) ?? adjustedTodayStart
            let monthStart = cal.dateInterval(of: .month, for: monthDate)?.start ?? monthDate
            displayMonths.append(monthStart)
        }

        for expense in dataManager.expenses {
            let expenseMonthStart = cal.dateInterval(of: .month, for: expense.date)?.start ?? expense.date

            // Check if expense month matches any of our display months exactly (including year)
            if displayMonths.contains(expenseMonthStart) {
                monthlyTotals[expenseMonthStart, default: 0] += expense.amount
            }
        }
        ...
        for i in 0..<6 {
            ...
            let amount = monthlyTotals[monthStart] ?? 0
        ...
        NavigationView {
            ZStack {
                ...
                ScrollView {
                    ...
                    // Graph Card with period indicator as title inside
                    VStack(spacing: 12) {
                        // Period indicator as title with navigation arrows inside card
                        HStack(spacing: 12) {
                        ...
                        VStack(spacing: 16) {
                            ...
                            if !periodExpenses.isEmpty {
                                ...
                                VStack(alignment: .leading, spacing: 12) {
                                    ...
                                    let categoryGroups = Dictionary(grouping: periodExpenses) { $0.effectiveCategory }
                                    ...
                                    VStack(spacing: 12) {
                                        ForEach(Array(categoryGroups.enumerated()), id: \.offset) { categoryIndex, categoryGroup in
                                            ...
                                            let percentage = selectedData.1 > 0 ? (categoryAmount / selectedData.1) * 100 : 0
                                            ...
                                            VStack(alignment: .leading, spacing: 8) {
                                                ...
                                                VStack(spacing: 6) {
                                                    ForEach(categoryExpenses, id: \.id) { expense in
                                                        SpendingGraphExpenseRow(expense: expense, categoryColor: category.color)
                                                            .onTapGesture {
...
struct SimpleLineGraph: View {
            ...
            GeometryReader { geometry in
                ...
                ZStack {
                    ...
                    ForEach(Array(getBudgetResetInfo().enumerated()), id: \.offset) { _, resetInfo in
                        ...
                        .stroke(Color.colAccent.opacity(0.8), lineWidth: 2)
                    ...
                    if !data.isEmpty && data.count > 1 {
                        ...
                        Path { path in
                            ...
                            // Close the path to the bottom to create filled area
                            let lastX = startX + (chartWidth * CGFloat(data.count - 1) / CGFloat(max(data.count - 1, 1)))
                            path.addLine(to: CGPoint(x: lastX, y: startY + chartHeight))
                            path.addLine(to: CGPoint(x: startX, y: startY + chartHeight))
                            path.closeSubpath()
                        }
                        .fill(
                            LinearGradient(
                                gradient: Gradient(colors: [
                                    Color.colAccent.opacity(0.3),
                                    Color.colAccent.opacity(0.1),
                                    Color.colAccent.opacity(0.0)
                                ]),
                                startPoint: .top,
                                endPoint: .bottom
                            )
                    ...
                    if let selectedIndex = selectedPointIndex, selectedIndex < data.count {
                        ...
                        ZStack(alignment: bubbleInfo.tailPointsUp ? .top : .bottom) {
                            ...
                            VStack(alignment: .center, spacing: 2) {
                                ...
                                if selectedData.1 == 0 {
                                    Text("\(String.currencySymbol())0.00")
                                        .font(.caption)
                                        .fontWeight(.semibold)
                                        .foregroundColor(.colPrimaryText)
                                } else {
    ...
    private func formatValue(_ value: Double) -> String {
        if value >= 1_000_000_000 {
            let bValue = value / 1_000_000_000
            let rounded = round(bValue * 10) / 10
            // Show one decimal only if significant (not .0)
            if rounded.truncatingRemainder(dividingBy: 1) == 0 {
                return String(format: "%.0fb", rounded)
            } else {
                return String(format: "%.1fb", rounded)
            }
        } else if value >= 1_000_000 {
            let mValue = value / 1_000_000
            let rounded = round(mValue * 10) / 10
            // Show one decimal only if significant (not .0)
            if rounded.truncatingRemainder(dividingBy: 1) == 0 {
            ...
            let rounded = round(kValue * 10) / 10
            // Show one decimal only if significant (not .0)
            if rounded.truncatingRemainder(dividingBy: 1) == 0 {
...
// MARK: - Expense Detail Row for Spending Graph
struct SpendingGraphExpenseRow: View {
    let expense: Expense
