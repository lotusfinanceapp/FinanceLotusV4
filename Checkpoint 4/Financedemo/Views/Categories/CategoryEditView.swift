...
struct CategoryEditView: View {
    ...
    // Filter icons - remove already used ones (except current category's icon if editing)
    private var filteredIcons: [String] {
        let usedIcons = categoryManager.allCategories
            .filter { existingCategory == nil || $0.id != existingCategory?.id }
            .map { $0.icon }
        return availableIcons.filter { !usedIcons.contains($0) }
    }

    // Filter colors - remove already used ones (except current category's color if editing)
    private var filteredColors: [String] {
        let usedColors = categoryManager.allCategories
            .filter { existingCategory == nil || $0.id != existingCategory?.id }
            .map { $0.colorHex }
        return availableColors.filter { !usedColors.contains($0) }
    }
                    ...
                    VStack(alignment: .leading, spacing: 25) {
                        ...
                        VStack(alignment: .leading, spacing: 15) {
                            ...
                            TextField("", text: $categoryName)
                                .placeholder("Enter category name", text: $categoryName)
                        ...
                        VStack(alignment: .leading, spacing: 15) {
                            ...
                            LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 8), spacing: 12) {
                                ForEach(Array(filteredColors.enumerated()), id: \.element) { index, colorHex in
                                    Button(action: {
                                        hideKeyboard()
                                        withAnimation(.easeInOut(duration: 0.2)) {
